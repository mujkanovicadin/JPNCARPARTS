...
test("home page loads and shows the platform heading", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: /genuine japanese automotive parts/i })
  ).toBeVisible();
});
...
test("golden path: browse catalog, view a product, add it to the cart", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "HKS Super SQV4 Blow Off Valve" })
  ).toBeVisible();
  ...
  await expect(page.getByText("HKS Super SQV4 Blow Off Valve")).toBeVisible();
});
